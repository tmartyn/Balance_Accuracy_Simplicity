...
for (d in datas) { # for each data type
  ...
  data.length<-dim(fecundity.data.W)[1]
...
sum<-unique(sum)
sum<-sum[-dim(sum)[1],]
write.csv(sum,paste0("Species.grouping/Rare.grouping/output/by.focal/rare.grouped.by.species.kfold.2019.csv"),row.names = FALSE)
