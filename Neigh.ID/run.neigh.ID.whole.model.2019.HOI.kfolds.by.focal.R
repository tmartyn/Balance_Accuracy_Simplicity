...
for (D in datas){
    ...
    for (t in mode.types) {
      ...
      for (k in 1:5){
        ...
        if (beta==F) {
          ...
          saveRDS(alpha.models.AB,paste0("Neigh.ID/output/by.focal/",D,".alpha.model.list.HOI.",t,"fold.",k,"focal.",f,".2019.rds"))
        ...
        if (beta==T) {
          ...
          saveRDS(alpha.beta.models.AB,paste0("Neigh.ID/output/by.focal/",D,".alpha.beta.model.list.HOI.",t,"fold.",k,"focal.",f,".2019.rds"))  
  ...
  ## write summary csv
  write.csv(sum,paste0("Neigh.ID/output/by.focal/",D,".neighbor.ID.summary.2019.HOI.kfolds.by.focal.csv"))
  
} # close for each data D

print("Neighbor ID analysis done - HOI-inclusive")