...
#rename columns and to prep combine data
...
# Make figures
#################### AUstralia ###################
CairoPDF("figures/Australia_neigh_ID.pdf",height=10,width=10)
...
ggplot(data=m.grouped.3[which(m.grouped.3$Data=="Australia"),],aes(x=Metric.direct.mean,y=Metric.HOI.mean,col=group))+
...
CairoPDF("figures/Spain_neigh_ID.pdf",height=10,width=10)
dummy <- data.frame(Metric.direct.mean = c(13400,16500, 13200,14000,1.05,1.5,0.5,1.6),
                    ...
                    Metric.name = c("BIC", "BIC", "AIC", "AIC","RMSE training","RMSE training","RMSE testing","RMSE testing"),
                    group=rep("Life Form",8))
ggplot(data=m.grouped.3.S[which(m.grouped.3.S$Data=="Spain"),],aes(x=Metric.direct.mean,y=Metric.HOI.mean,col=group))+
  geom_point(shape=21,size=5,stroke=1.5)+geom_abline(slope=1)+
  facet_wrap(vars(Metric.name), scales="free")+
  geom_blank(data=dummy)+
  geom_errorbarh(aes(xmin=Metric.direct.mean-2*Metric.direct.sd,xmax=Metric.direct.mean+2*Metric.direct.sd))+
